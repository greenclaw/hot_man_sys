...
shell.exec(`${prefix} -d hot_man_sys -c "\\COPY room_types(class_name,capacity,bed_num) FROM '${__dirname}/db/rooms_types.csv'  WITH DELIMITER ',' csv;"`)
...
shell.exec(`${prefix} -d hot_man_sys -c "\\COPY hotel_owners       FROM '${__dirname}/db/owners_hotels.csv'       WITH DELIMITER ',' csv;"`)
