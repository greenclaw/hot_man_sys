...
shell.exec(`${prefix} -d hot_man_sys -c "\\COPY accounts(first_name,last_name,user_name,email,user_password)    FROM '${__dirname}/db/accounts.csv'       WITH DELIMITER ',' csv;"`)
shell.exec(`${prefix} -d hot_man_sys -c "\\COPY owners(first_name,last_name,user_name,email,user_password)       FROM '${__dirname}/db/owners.csv'       WITH DELIMITER ',' csv;"`)
shell.exec(`${prefix} -d hot_man_sys -c "\\COPY owners_hotels       FROM '${__dirname}/db/owners_hotels.csv'       WITH DELIMITER ',' csv;"`)
...
shell.exec(`${prefix} -d hot_man_sys -c "\\COPY managers(first_name,last_name,user_name,salary,email,phone_number,hotel_id,user_password)   FROM '${__dirname}/db/managers.csv'        WITH DELIMITER ',' csv;"`)
