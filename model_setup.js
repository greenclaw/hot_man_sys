...
shell.exec(`${prefix} -d hot_man_sys -c "\\COPY hotels(hotel_name, star_num, price_id, city, country_code, country, address, url, rating)       FROM '${__dirname}/db/hotels.csv'       WITH DELIMITER ',' csv;"`)
...
shell.exec(`${prefix} -d hot_man_sys -c "\\COPY guests(first_name, last_name, age, phone, email, guest_password)                             FROM '${__dirname}/db/guests.csv'       WITH DELIMITER ',' csv;"`)
